...
type Props = { params: { slug: string }; searchParams?: { invite?: string } };
...
export default async function WriteReviewPage({ params, searchParams }: Props) {
  const company = await getCompanyBySlug(params.slug);
  if (!company) notFound();
  const invitationToken = String(searchParams?.invite ?? "").trim();
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      ...
      <div className="mt-8">
        <ReviewForm slug={company.slug} invitationToken={invitationToken} />
      </div>
    </div>
  );
}
